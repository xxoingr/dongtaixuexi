...
last_updated: 2026-09-11T20:11:54+08:00
status: in-progress
---  
...
- [x] <!-- id:t_phase_f dep:t_phase_e --> Day 50-53: 版本 Diff + 特征码定位 + CE 进阶
- [ ] <!-- id:t_phase_g dep:t_phase_f --> Day 54-57: Windows 多线程 + 同步机制 + Hook 稳定性  
- [ ] <!-- id:t_phase_h dep:t_phase_g --> Day 58-60: 第二次综合项目 + 完整逆向报告  

## Current Status  

进度: 53/60 — 第一阶段 30/30 完成，第二阶段 Day 31-34、Day 35-38、Day 39-41、Day 42-45、Day 46-49、Day 50-53 完成（23/30）
路线: Day 8-60 主路线保持不变；固定链路为 DS 理论前置 -> 理论总结测试与主动查缺补漏 -> 用户明确同意进入实操 -> Sol 工程准备与后台验收 -> DS 用户实操教学/复盘/网站闭环；Day 46-60 四块工程已由 Sol 提前冻结并验收，匹配未改动产物时在理论总结门通过且用户明确同意后直接读取预制卡进入 DS 实操教学，省略重复工程请求/交回往返；Sol 另负责学习计划大方向、高难救场和每 3-5 课定期抽查
教学阶段: DS 理论前置
实操模型: Sol 负责实操工程制作与后台验收；DS 负责基础理论、用户实操教学、复盘与日常闭环；Sol 另负责大方向、救场和抽查
正在: Day 54-57 Windows 多线程 + 同步机制 + Hook 稳定性 — 前一块 Day 50-53（版本 Diff + 特征码定位 + CE 进阶）已于 2026-09-10 正式完成并闭环：Sol 五次八门审核（前四次共 20 处定点返修：ASLR 与 CheckSum 分层、AOB 扫不到因果、代码注入与指针链边界、必然化语义收口、绝对地址 vs「模块名 + RVA」口径）后八门全部通过，days.json 的 day50-53 已标记 done 并追加正式学习日志，node build.js 输出 39 文章单元、30/30 · 23/30（块 6/8）· 53/60，只暂存 SESSION_PROGRESS.md / days.json / index.html 三文件提交推送并核对线上页面。当前课程转 Day 54-57，教学阶段回到 DS 理论前置。
下一步: DS 开始 Day 54-57（Windows 多线程 + 同步机制 + Hook 稳定性）理论前置：先按合并块规则把本块拆成第 1/4 至第 4/4 小课并给出学习地图，再逐条讲解、逐条确认理解；四小课讲完后执行理论总结门（总结测试 + 主动查缺补漏 + 单独确认是否进入实操），随后启用 Sol 已冻结的 Day 54-57 预制卡进入 DS 实操教学。全站翻新审计按独立提交另行启动，不占本块进度。网站翻新的第二批第四次返修经 Sol 复核结论为“不通过——但只剩 1 处句子级技术口径（基础-1 自测 Q6 答案的绝对化说法）”，DS 已完成第五次句子级返修并新建本地提交（未 push），唯一下一步是送《给 Sol 的第二批第五次网站内容审核请求》；Sol 通过前不 push、不开始 Day 11～30。

## Decisions

- <!-- at:2026-09-11T20:11:54+08:00 --> 全站翻新第二批第五次（句子级）返修完成并新建本地提交（在 4934318 之后，未改写历史、未 push）：按 Sol 第四次审核的唯一修正点，只改基础-1 自测 Q6 答案里的一个绝对化句——旧句“只搜一次永远缩不到唯一地址；最后改值还能观察验证，才算真的找到。”改为“首次搜索通常会留下多个候选，需要继续制造变化来筛选；即使碰巧只剩一个，也要通过修改并观察验证，才算真的找到。”（题面与前半句“关键是中间那步‘制造变化再搜’”原样保留）。旧句全库命中 1 次、替换后 0 次，新句 days.json 与 index.html 各命中 1 次，新句不含 永远 / 必然 / 绝不可能 等绝对化词；字段级对比确认只改 foundation[0].html 一个字段，status / practice / log / phase2 / 标题 / 视频零改动；基础-1 自测仍 6 题。验证结果：node build.js 输出 39 文章单元、第一阶段 30/30、第二阶段 23/30（块 6/8）、总体 53/60；结构复验 13 篇各 1 个小目录 + 1 个六项入口、135 个 id 全唯一、站内断链 0、基础-1 / 基础-3 / Day 8 各 6 道自测、day50-53 仍 done、无 Day 54-57 正式文章（18 项检查全 PASS）；Playwright Chromium 复测 28 项全 PASS（基础-1 展开折叠答案后新句可见、旧句不出现、Q6 题面与主线仍在；基础-2 / 基础-3 / Day 5-10 每页仍恰好一个六项入口；390px 七页无横向溢出；无 console error / page error）；git diff --check 退出码 0。未 push、未开始 Day 11～30；网站翻新不构成学习证据，课程进度仍 53/60、当前课程仍 Day 54-57、教学阶段仍 DS 理论前置。唯一下一步：送《给 Sol 的第二批第五次网站内容审核请求》。

- <!-- at:2026-09-11T20:02:22+08:00 --> 全站翻新第二批第四次定点返修完成并新建本地提交（在 20a470f 之后，未改写历史、未 push）：按 Sol 第三次审核“不通过（最后 3 组）”的要求逐条收口，共 21 处句子级替换（基础-1 七处、基础-3 四处、Day 8 十处），21/21 全部通过“恰好命中一次”检查后才写回 days.json；字段级对比确认只改 3 个 html（基础-1、基础-3、Day 8），status / practice / log / phase2 / 标题 / 视频零改动。返修要点：①基础-1 数据类型表补齐 Byte（无符号 0 ~ 255；有符号 -128 ~ 127）与 2 Bytes（无符号 0 ~ 65,535；有符号 -32,768 ~ 32,767），表下解释统一为“Byte、2 Bytes、4 Bytes、8 Bytes 都只是整数宽度、都可能按有符号或无符号解释；Float 与 Double 是另一套浮点格式，字节数相同不代表类型相同”②基础-1 清理地址稳定性旧口径：30 秒入口“忘了先看哪里”改为判断“旧绝对地址重启后还能不能对应同一份数据 + 目标属于模块内静态位置还是运行时堆对象”，验证与证据表末行改成分情况处理（静态位置记模块名 + RVA / 堆对象继续找指针链 / 也可找 AOB/特征码锚点 / 原绝对地址只留作当次运行证据），与 Q1 答案一致③基础-1 自测覆盖修正：Q2 扩成同时问“Byte/2/4/8 Bytes 为什么都要考虑有符号与无符号”“同样 4 字节 4 Bytes 与 Float 为何不同”“同样 8 字节 8 Bytes 与 Double 为何不同”，并新增 Q6 要求复述 CE 主线（附加进程 → 搜当前值 → 制造变化 → 搜新值 → 反复筛选 → 修改并观察验证），基础-1 变 6 题④基础-3 消除正文与代码框冲突：标题改“本实验核心代码（简化版）”、小目录条目同步，并在代码框内 GetProcAddress 行上方加三行注释（这一行只是本课当前同系统靶场用过的简化写法、不是通用跨进程入口解析方案、通用实现必须解析实际承载模块 + Export RVA 与 Forwarder RVA）⑤基础-3 跨位数表述收紧为“本课这套直接传远程函数入口的简化实现要求注入器、目标进程和 DLL 位数一致；跨位数需要额外的桥接或专门实现，本课没有覆盖”⑥Day 8 给 Alertable State 结论加适用范围：一句话核心结论、最小执行链、k4、注入流程第 6 步、自测 Q4 与对比表“硬前提”行统一为“本课把普通用户态 APC 排给已经在运行的线程；这条路径里要等线程进入 Alertable State 才执行”⑦Day 8 k4 补微软文档写出的启动前例外（APC 若在线程开始运行前排入队列，线程启动时会先调用它；不属于本课已运行线程路径，结论不能外推）⑧Day 8 补跨进程使用边界（微软不建议把 QueueUserAPC 排到调用方进程之外的线程；DLL 重定位、函数地址、位数不一致都可能失败或崩溃；本课只是自建同架构靶场的机制演示，远程导出解析不能抹掉这条官方边界）⑨Day 8 把“六种注入对比”标题改为“到 Day 8 为止的五种注入方式对比”（未提前加 Day 9 线程劫持），行名“需要句柄”收紧为“是否需要主动持有目标进程/线程句柄”⑩Day 8 新增边界自测（为什么已运行线程要进 Alertable State、文档例外、为什么跨进程 QueueUserAPC 不能当通用推荐），Day 8 变 6 题。验证结果：node build.js 输出 39 文章单元、第一阶段 30/30、第二阶段 23/30（块 6/8）、总体 53/60；结构复验 13 篇各 1 个小目录 + 1 个六项入口、135 个 id 全唯一、站内断链 0、基础-1 与 Day 8 各 6 道自测、day50-53 仍 done、无 Day 54-57 正式文章；旧句（六种注入对比 / 32 位注入器打 64 位目标会直接失败 / 是不是重启就失效的动态地址 / 注入器核心代码等）在生成页命中 0；Playwright Chromium 复测 34 项全 PASS（含折叠答案展开后的内容断言、f3-k3 与 day8-k4 锚点跳转落点、390px 三页含展开态无横向溢出、无 console error / page error）；git diff --check 退出码 0。未 push、未开始 Day 11～30；网站翻新不构成学习证据，课程进度仍 53/60、当前课程仍 Day 54-57、教学阶段仍 DS 理论前置。唯一下一步：送《给 Sol 的第二批第四次网站内容审核请求》。

- <!-- at:2026-09-11T19:38:31+08:00 --> 全站翻新第二批第三次定点返修完成并新建本地提交（在 a709d15 之后，未改写历史、未 push）：按 Sol 第二次审核的 6 组定点返修要求逐条落盘，共 20 处句子级替换 + 1 处代码块重写 + 1 道自测补题（21 项全部通过“恰好命中一次”检查后才写回 days.json）；字段级对比确认本次只改 6 个 html 字段（基础-1、基础-3、Day 2、Day 5、Day 6、Day 8），status / practice / log / phase2 / 标题 / 视频零改动。返修要点：①基础-1 自测 Q1 答案收紧——本来就是模块内静态位置才记“模块名 + RVA”，堆对象要继续找稳定指针链，也可到访问这段数据的代码附近找 AOB/特征码锚点，某次运行的绝对地址只留作“带版本 + 当次运行”的证据，不再统一写成“模块名 + 偏移（或特征码）”②基础-3 权限段改为两层口径：第一层按操作对权限（VirtualAllocEx → PROCESS_VM_OPERATION、WriteProcessMemory → PROCESS_VM_WRITE、创建远程线程 → PROCESS_CREATE_THREAD），第二层写明 CreateRemoteThread 文档列出的句柄权限集合（PROCESS_CREATE_THREAD、PROCESS_QUERY_INFORMATION、PROCESS_VM_OPERATION、PROCESS_VM_WRITE、PROCESS_VM_READ），删除“因为常要读所以不是可有可无”的理由，Q2 同步删除“按需读取”口径③基础-3 演进路线 Day 7 行改为“不走 OpenProcess → VirtualAllocEx → WriteProcessMemory → CreateRemoteThread 这条主动注入链；但满足 Hook 条件时，系统仍会把 Hook DLL 加载进目标，并在目标线程上下文执行回调”④基础-3 与 Day 8 的远程 LoadLibrary 地址统一补导出转发口径：普通 Export RVA 才用“实际承载模块基址 + RVA”，Forwarder RVA 要顺着转发找到真正承载实现的模块，不得因为名字属于 kernel32.dll 就无条件加它的基址⑤基础-3 新增自测 Q6“LoadLibrary 注入失败时按哪几类问题排查”，答案覆盖编码与 A/W 版本、三者位数、权限与跨会话、DllMain/Loader 锁、远程入口与导出转发⑥Day 2 统一为“五个定位/换算字段（Name、VirtualSize、VirtualAddress、SizeOfRawData、PointerToRawData）＋另外还要看 Characteristics”，消除“第六个关键字段”的误读⑦Day 5 删除快速对比表“触发 LoadImageNotify 回调？✅/❌”整行（本课没有做内核镜像加载回调实验，不保留无边界勾叉）⑧Day 6 代码块整体重写为遍历逻辑（读模块名 → 匹配 kernel32.dll → 取 DllBase），代码框外保留“本次 Win10 x64 观察到 EXE → ntdll → kernel32 的顺序”并注明不是执行步骤；自测 Q4 改答“偏移与次序是当前环境观察值、实际工具要遍历按模块名匹配”；一句话核心结论与 k1 收紧为“本课这个 Shellcode 选择从 PEB 找模块、自行解析导出（Shellcode 也可以由调用方直接传入 API 地址）”⑨Day 8 k4 改为“本课边界：只讲 QueueUserAPC 这一类 APC”，删除普通内核 APC / 特殊内核 APC 的错误交付条件表述，小目录条目同步；自测 Q5 改问“本课为什么只围绕普通用户态 APC 讲 Alertable State、其他类型能不能套用同一条件”；自测 Q1 题面改为“怎么判断一次等待是否请求了 Alertable Mode”，答案分 bAlertable 布尔参数与 MWMO_ALERTABLE 标志两类⑩Day 8 远程 LoadLibraryW 说明与基础-3 同口径。验证结果：node build.js 输出 39 文章单元、第一阶段 30/30、第二阶段 23/30（块 6/8）、总体 53/60；结构复验 39 个六项入口齐全、135 个 id 全唯一、站内断链 0、day50-53 仍 done、无 Day 54-57 正式文章、基础-3 自测 6 题；Playwright Chromium 复测 40 项全 PASS（含展开折叠答案后的自测内容断言、6 个页面 390px 无横向溢出）；git diff --check 通过，工作区仍只有 SESSION_PROGRESS.md / days.json / index.html 三文件。未 push、未开始 Day 11～30；网站翻新不构成学习证据，课程进度仍 53/60、当前课程仍 Day 54-57、教学阶段仍 DS 理论前置。唯一下一步：送《给 Sol 的第二批第三次网站内容审核请求》。

- <!-- at:2026-09-11T18:14:31+08:00 --> 全站翻新第二批第二次（基础-1～3、Day 1～10 共 13 篇）定点返修完成并新建本地返修提交：按 Sol 上一轮不通过点与四项补充意见逐条落盘，共 34 处定点替换 + 13 篇自测补题，全部通过“旧文本恰好命中一次”检查（34/34 命中 1 次）后才写回 days.json；写回前后字段级对比确认只改了 13 个 html 字段，status / practice / log / phase2 / title / video 零改动。返修要点：①基础-1 把 8 Bytes 与 Double 拆开——8 Bytes 是 64 位整数（有符号约 -9.22×10^18 ~ 9.22×10^18、无符号约 0 ~ 1.84×10^19），Double 是独立的 64 位浮点扫描类型；Float 改为“4 字节浮点数”，不再只讲成“小数”②基础-2 指针结构图“基址（不变）”改为“模块基址 + RVA（同一份构建里可复用，仍要复核）”③基础-3 把 PROCESS_VM_READ 明确写成“不是可有可无”（注入器常要读远程 PEB、模块列表或回读写入结果），演进路线 Day 6 / Day 7 两行改为“裸机器码，没有 PE 结构、也不依赖额外的 DLL 文件”与“不直接操作目标进程，由系统把 Hook DLL 加载进去”④Day 1 示意图“强制校验位”改为“签名，解析链的入口凭证”⑤Day 5 对比表两行改为“触发 LoadImageNotify 回调？”与“痕迹对比”，并把 day5-k5 锚点从“已验证结论与调试证据”移到真正的“实测输出与检测边界”标题上⑥Day 6 PEB 走法标注“第 1/2/3 项只是本实验环境里的示例顺序”并补“遍历链表按名字匹配”的提醒，对比表“不需要任何文件”改为“不需要额外的 DLL 文件（字节码由注入器直接写入内存）”，载荷外形同步改为“没有 PE 结构、没有模块登记”⑦Day 7 删除“隐蔽性排序 4/1/2/3”固定数字排名，改为“主要痕迹（检测视角）”⑧Day 8 新增等待 API 表（SleepEx 第 2、WaitForSingleObjectEx 第 3、WaitForMultipleObjectsEx 第 5、SignalObjectAndWait 第 4 个 bAlertable；MsgWaitForMultipleObjectsEx 没有独立布尔参数、用 dwFlags 的 MWMO_ALERTABLE），踩坑表删除“DLL 永不加载”“需目标线程调用 *Ex 系列 API”，内核 APC 标注为理论边界⑨Day 10 声明与实际统一为 6 个知识点，观察表补独立锚点 day10-k2b⑩13 篇逐篇补自测题让每个目录知识点至少对应一道真实问题（基础-1/2/3、Day 1-9 各 5 题，Day 10 共 6 题），Day 9 同步把证据表的“线程起始地址”与“已有线程当前 RIP”拆成两行。验证结果：node build.js 输出 39 文章单元、第一阶段 30/30、第二阶段 23/30（块 6/8）、总体 53/60；结构复验 39 个 quick-review 区块各恰好一个且六项齐全、重复 id 0、站内断链 0、day50-53 仍为 done、无 Day 54-57 正式文章（仅路线图“计划中”标记与前后关联文字提及）；Playwright Chromium 复测 42 项全 PASS（基础-1/3 与 Day 5/6/7/8/10 定点断言、day5-k5 小目录点击跳转、390px 七页无整页横向溢出）；git diff --check 通过，工作区仍只有 SESSION_PROGRESS.md / days.json / index.html 三文件。未 push、未开始 Day 11～30；网站翻新不构成学习证据，课程进度仍 53/60、当前课程仍 Day 54-57、教学阶段仍 DS 理论前置。唯一下一步：送《给 Sol 的第二批第二次网站内容审核请求》。

- <!-- at:2026-09-11T03:05:00+08:00 --> 全站翻新第二批（基础-1～3、Day 1～10 共 13 篇）已完成并提交 b539a34，Sol 审核结论为「不通过，定点返修」（网站内容审核，不改变课程路线、Day 完成状态和学习证据）：本批为 13 篇各补《本课知识点小目录》与「3 分钟自测」，并定点修正技术边界（动态地址表述、类型宽度分层、指针链候选路径、节区归属非固定、OriginalFirstThunk 回退、重定位只修需修正位置、Manual Map 入口语义与「跳过 DllMain」为本实验设计、Shellcode 位置无关条件与导出转发、全局 Hook 范围、APC 交付条件、线程劫持完整现场保存等）；b539a34 自检结果为 39 个页面各恰好一个六项入口、14 篇有小目录、13 篇有自测、重复 id 0、断链 0，进度仍 53/60。Sol 复核确认结构、导航、移动端与大部分通俗表达合格，不通过点为：①基础-1 的 8 Bytes 与 Double 必须拆开、Float 不能只讲成「小数」②基础-2 代码框仍写「基址（不变）」③基础-3 的 CreateRemoteThread 权限集合口径（PROCESS_VM_READ 不是可有可无）与路线图两条表述④Day 1 示意图仍写「强制校验位」⑤Day 5 对比表的 LoadImageNotify/隐蔽性两行与 day5-k5 锚点落点⑥Day 6 的 PEB「第一/二/三项」固定次序示例与「不需要任何文件」⑦Day 7 固定数字排名⑧Day 8 的 APC 参数与交付条件（SleepEx 第 2、WaitForSingleObjectEx 第 3、WaitForMultipleObjectsEx 第 5 个 bAlertable；MsgWaitForMultipleObjectsEx 用 MWMO_ALERTABLE 标志；失败表不得写「永不加载」「必须调用 Ex 系列」；内核 APC 移到理论边界）⑨Day 10 声明 5 个知识点却列 6 条⑩部分篇目自测未覆盖全部目录知识点。DS 已按卡写好定点返修补丁（13 篇全部替换 + 覆盖补题，脚本在临时目录 apply_batch2fix.js），但本次会话的执行通道被宿主 read-evidence 守卫拦截（bash 连续被拒并触发循环守卫，要求停止重试），返修尚未落盘。day50-53 仍为 done、进度仍为 53/60、主课程仍为 Day 54-57、教学阶段仍为 DS 理论前置；网站翻新不构成学习证据。唯一下一步：在具备写权限的会话中运行 apply_batch2fix.js → node build.js → 静态与浏览器复验 → 更新本 SESSION → 新建本地返修提交（不改写 b539a34）→ 送《给 Sol 的第二批第二次网站内容审核请求》；在 Sol 通过前不 push、不开始 Day 11～30。
...
- <!-- at:2026-09-10T23:28:10+08:00 --> Sol 第二次八门审核仍判 Day 50-53「不通过」：主体方案、三段用户证据、复习结构与老师式表达均已合格，唯一缺口是「返修没有全域收口」——正文仍残留几句旧的绝对化结论、一个旧版复习口诀，以及一条不属于学习内容的 Reasonix 环境记录。DS 完成最后七处定点收口：①A 节「即便基址固定，重编译照样让偏移失效」→「重编译仍可能改变偏移，旧偏移不能未经核对继续使用」②B 节执行链不再「先划掉 PE 头和调试目录的红」，改为先确认红点对应哪个具体字段、只有确认是 TimeDateStamp/RSDS-PDB 这类不解释功能变化的记账信息才先排除，并写明 PE 头里的入口点/标志/目录位置可能影响程序行为、不能整片划掉；B 节适用边界删除「同一份源码重新编译的必然产物」，改为「是否出现、是否填写、这次是否变化取决于工具链与构建配置；本块实测只有 TimeDateStamp 与 RSDS/PDB 信息不同，CheckSum 四份均为 0」③学习地图「什么时候必须扫内存」与 S 第 7 题「为什么往往必须扫内存」统一改为「什么时候更适合等运行时解包后扫内存、什么时候可对已还原映像离线扫描」；H 节「模块 + 偏移」补上「同一份构建、相同二进制」前提④J 节「每次都会变的东西」、O 表「在新版本失效／重编译会让偏移变化」、Q 节「直接失效／绝对地址作废」、S 第 1 题「重编译会让偏移变」统一改为「可能改变／不保证稳定／未经复核不能继续依赖」；O 表行名改为「记『基址 + 偏移』的笔记换版本后可能失效」，原因注明本块 V1/V2 相同只说明这次没有挪动该数据、不代表其他版本也会保持⑤R 3 分钟口诀补上「扫不到先查进程/版本/模块、扫描类型/范围/过滤器以及是否已加载或解包，确认字节确实改变后再回附近做定向 Diff」⑥Q 节删除没有用户证据或对抗实测支撑的「防守方也可以借此判断对方是否在用自动化比对流水线」，并把重新编译/重新加载改写为「可能改变布局、基址不保证与上次相同，只写死地址的工具不可靠」⑦Day 46-49 学习日志删除 Reasonix 宿主/bash/PowerShell 代跑等软件环境与协作过程记录（此类问题留在 SESSION 或维护者问题总结中，不进入面向复习的网站日志），保留键盘媒体键 Fn+W/F12 的用户操作坑。node build.js 仍输出 39 文章单元、30/30 · 19/30（块 5/8）· 49/60；41 项本地复验全 PASS（第一轮 13 条 + 第二轮 9 条旧口径检索全为 0，B/H/R/S/Q 与日志清理逐项确认）；git diff --check 退出码 0、git status 仍只有 SESSION_PROGRESS.md / days.json / index.html 三文件、build.js 未修改、四个冻结产物未变动；未标记 done、未提交、未推送、未启动全站翻新。
- <!-- at:2026-09-10T23:16:19+08:00 --> Sol 第一次八门审核判定 Day 50-53「不通过」，DS 完成七项定点返修并本地复验：Sol 只读复核确认生成与进度口径、index.html 重新生成前后 SHA-256 相同（deterministic）、工作区只有三文件、10 条小目录与 k1-k10 锚点、四个冻结产物哈希与 x64/AOB 唯一性、DYNAMIC_BASE 与 TimeDateStamp、RSDS/PDB 记录、可见日志最新条目；八门中覆盖门/可复核门/复习门通过，讲透门/证据门/失败门/一致性门/独立恢复门不通过。返修点：①ASLR 统一收紧为「允许重定位、不保证与上次相同」，删除「每次重启都换」「版本变化时基址一定会变」等绝对化表述（入口一句话结论与最小执行链、A 节、N 表、O 表、Q、R、S 第 1 题、T）②PE 记账信息分层：TimeDateStamp 与调试目录（RSDS/PDB GUID 与路径）本块四次构建实测确实不同，CheckSum 是 PE 头独立字段且本块四个产物实测全为 0x00000000，不得写成必然变化的噪音（入口最易混点、B 节、N、O、Q、S 第 2 题）③AOB 扫不到改为按序排查（错进程/错版本/错模块 → 扫描类型、范围、保护过滤器 → 模块未加载或未解包 → 特征字节真被改），删除「被挪走所以扫不到」错误因果（入口最小执行链、F、J、O、S 第 10 题）④重写 I 节代码注入：常见 AA 注入把注入点若干字节改成跳往 code cave 的跳转、可为保持原逻辑重放被覆盖原指令也可有意不执行、运行时 detour 不会让原 EXE 后方指令整体后移（0x11 后移只属 V1/V2 两个独立编译产物的布局差异）⑤指针链改为「候选路径可能在同一构建的多次启动之间稳定、仍需重启复查与跨版本复核」；加壳统一为「磁盘文件通常不是业务代码的最终形态，常见走法是等运行时解包后扫内存或先取得已脱壳/还原的映像离线扫；『等它自己解开』不等于完整脱壳过程」（H、J、N、O、S 第 7 题）⑥O 表十六进制故障改为「抄错/漏字节/字节数不对」并注明大小写不改变字节值⑦log 数组补齐 Day 35-38（2026-08-30）、Day 39-41（2026-08-31）、Day 46-49（2026-09-09），Day 42-45 日期由 2026-08-31 校正为 2026-09-01，内容均从既有正式完成记录提炼、未新增学习证据，Day 50-53 暂不写正式完成日志。返修后 node build.js 仍为 39 文章单元、30/30 · 19/30（块 5/8）· 49/60，25 项本地验证全 PASS（含 13 条错误口径检索），git diff --check 退出码 0、git status 仍只有 SESSION_PROGRESS.md / days.json / index.html 三文件、build.js 未修改；改动统计按返修后的实际 git diff --numstat 记录（见第二次审核请求卡），原「255 insertions」口径作废。day50-53 保持 in-progress，未提交未推送，未启动全站翻新。
- <!-- at:2026-09-10T22:49:30+08:00 --> Day 50-53 本地闭环完成、进入 Sol 八门审核门：按 Sol 交接卡，DS 先重读长期规则、主 SESSION、days.json、build.js、index.html 与靶场 SESSION，确认三段实操与复盘已结束，再只执行第一阶段。days.json 新增 day50-53 合并块（正文 A-T 章节：两层漂移 / 记账噪音 / 连带差异 / 两种比对与读法 / 三条标准 / 两道关与命中数分诊 / 通配符 / 文件与内存扫描及加壳边界 / CE 四件事 / 跨版本工作流 / V1-V2 行为对照 / CE 取证与 0 命中排错 / x64dbg 两版反汇编对照 / 跨概念对照表 / 失败排错 / 证据等级 / 逆向与防御视角 / 3 分钟复习 / 10 题自测 / 实验环境与版本边界；practice 记录三段取证），status 先记 in-progress（发布前不计入覆盖天数）。node build.js：39 文章单元、第一阶段 30/30、第二阶段 19/30（块 5/8）、总体 49/60；53 项本地验证全 PASS；git diff --check 通过；工作区只改了 SESSION_PROGRESS.md / days.json / index.html 三文件，未提交未推送。已知一致性缺口：days.json 的 log 数组自 2026-08-31（Day 42-45）后未再追加，Day 35-38 / 39-41 / 46-49 与本块都没有日志条目，提交 Sol 判断是否统一补写。未启动全站翻新，未提前写 Day 54-57。

- <!-- at:2026-09-10T21:59:30+08:00 --> 用户确认长期目标是掌握每个知识点并方便日后复习，采用“知识点全覆盖、相关点可合并提问、题数服从覆盖度、网站完整列点但不强拆空壳章节”的口径。`更新笔记助手.txt` 已把《本课知识点小目录》纳入 Sol 八门中的复习门和发布前验证基线，明确目录与正文双向覆盖、相关知识点可共享实质章节锚点，并修正预制卡规则中残留的“理论后直接进入实操”表述。本次仅为教学与网站结构规则，不构成 Day 50-53 学习证据；本块 10 个知识点已由总结三问与两道查漏题全部覆盖并通过，当前待回答问题为用户是否仍有眼生或需补讲的点。status 仍 in-progress，plan 仍 24/36，进度仍 49/60，教学阶段仍为 DS 理论前置。

- <!-- at:2026-09-10T21:31:18+08:00 --> 用户授权把“理论总结测试 -> 主动查缺补漏 -> 单独询问是否进入实操”固化为所有 Day 的长期阶段门。`更新笔记助手.txt` 已补齐查缺补漏与最小再确认要求，并统一两处旧的“理论结束后直接进入实操”表述；不得把“没有待补充问题”视为用户已同意进入实操。Day 50-53 已完成的四小课理论、预制卡与冻结产物 SHA-256 核对继续有效，不重复教学或工程验收；因本块尚未完成新阶段门，教学阶段校正为 DS 理论前置，下一步仅为第一个理论总结短问题。status 仍 in-progress，plan 仍 24/36，进度仍 49/60，Day 50-53 未完成。

- <!-- at:2026-09-10T21:15:41+08:00 --> Day 50-53 理论前置完成并启用预制卡：DS 按合并块规则拆四小课逐条讲解并确认用户理解（Day 50 版本 Diff：地址漂移两层原因 ASLR+重编译/记账信息噪音/全量 vs 定向比对/节区定位与连带差异；Day 51 特征码定位：唯一+稳定+够短/命中数分诊/通配符 ??/文件与内存扫描/加壳；Day 52 CE 进阶：蹲写入者/指针链/代码注入=Inline Hook/结构体分析；Day 53 跨版本重定位工作流），未提前展开后续 Day。读取 `docs\prebuilt_handoffs\DAY50_53.txt` 并用 Get-FileHash 核对四个冻结产物 SHA-256 与 PACKAGE_MANIFEST.sha256 完全一致（Debug V1 c229983e…、Debug V2 fb08dfac…、Release V1 0d32638b…、Release V2 ef245053…），预制卡有效，不生成重复的《给 Sol 的实操工程请求》。教学阶段由 DS 理论前置改为 DS 实操教学；status 仍 in-progress，plan 仍 24/36，进度仍 49/60，Day 50-53 未完成。用户待亲手取得的证据：Release V1/V2 行为对照（1050 vs 1125）、CE 的 16 字节 AOB 在双版本命中与重启漂移复查、x64dbg 中 RslDay50ScoreAward 的 V1/V2 反汇编差异。
...
- <!-- at:2026-08-31T21:30:00+08:00 --> Day 39-41 正式完成并闭环：DS 按 Sol 20:00 优化方向卡，将 Day 31-34、Day 35-38 结构化扩写去重（分概念章节 + 3 分钟复习 + 自测），并新增 Day 39-41 正式文章（按 A-K 结构：架构总览/SEH/VEH 两路径/x64 表驱动展开/TLS-Loader/靶场变体/失败排错/证据等级/9 题自测）；三块 practice 统一结构并补版本边界（RVA 当次构建值、vptr 当次 ASLR 值、旧命令行问题归为历史失败实验）。node build.js 验证通过（36 文章单元、36 复习入口、进度口径 30/30·11/30·41/60），git diff --check 与 node --check 通过，第九项 13 条自验 ALL PASS，无 Day 42-45 页面、无独立 .xdata 节声称、改命令行仅在失败语境。仅暂存 SESSION_PROGRESS.md/days.json/index.html 三文件提交 a29dfd8 推送，线上 Pages 已核对（HTTP 200、41/60 进度、三篇可访问）。t_phase_c 标记完成，plan 21/36→22/36，进度 41/60，当前课程转 Day 42-45，教学阶段 DS 理论前置。未提前展开或完成 Day 42-45。
